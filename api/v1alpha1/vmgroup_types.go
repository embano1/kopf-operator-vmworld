...
type VmGroupStatus struct {
	...
	VmOperator VmOperatorStatus `json:"vm_operator"`
}

type VmOperatorStatus struct {
...
// +kubebuilder:subresource:scale:specpath=.spec.replicas,statuspath=.status.vm_operator.desiredReplicas
// +kubebuilder:resource:shortName={"vg"}
// +kubebuilder:printcolumn:name="Phase",type=string,JSONPath=`.status.vm_operator.phase`
// +kubebuilder:printcolumn:name="Current",type=integer,JSONPath=`.status.vm_operator.currentReplicas`
...
// +kubebuilder:printcolumn:name="Last_Message",type=string,JSONPath=`.status.vm_operator.lastMessage`
